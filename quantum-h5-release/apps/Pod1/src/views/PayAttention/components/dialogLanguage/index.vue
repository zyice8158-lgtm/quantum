    ...
    <template #header>
      <div class="flex w-full justify-between items-center">
        <div class="flex items-center">
          <span class="text-[16px] font-bold">{{ t('payAttention.languageSettings') }}</span>
        </div>
      </div>
    </template>
    <template #default>
      <div class="flex w-full mb-[8px] justify-between items-center text-[14px]">
        <span>{{ t('payAttention.source') }}</span>
      ...
      <div class="flex w-full justify-between items-center text-[14px]">
        <span>{{ t('payAttention.target') }}</span>
        <QIcons name="target" />
        <Select
...
import { getLanguageList } from "./index.ts";
...
const sourceLanguage = ref("auto"); // 源语言
