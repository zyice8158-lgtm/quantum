<template>
<div class="QSelectParent inline-block" :class="$attrs.class">
  <span class="label pl-[16px] pt-[12px]" v-if="labelFlag"> {{ placeholder }}</span>
  <Select
      unstyled
      :pt="theme"
      :ptOptions="{
      ...
      @focus="handleInput"
      @blur="handleBlur"
      v-bind="{...$attrs, class: 'w-full h-full'}"
...
import { useSlots,ref,defineProps,toRef } from 'vue';
import { ptViewMerge } from "../utils";
const slots = useSlots();
console.log(slots,36)
...
const dropdownClass = ` shrink-0 bg-transparent text-surface-400 w-10 rounded-e-md`
function handleInput(value: any) {
  // labelFlag.value = true
  theme.value.dropdown = dropdownClass+' flex items-start '
  placeholderRef.value = ''
}
function handleBlur() {
  theme.value.dropdown = dropdownClass+' flex items-center justify-center '
...
const theme = ref<SelectPassThroughOptions>({
    root: `inline-flex cursor-pointer relative select-none rounded-md p-fluid:flex
        ...
        transition-colors duration-200 pl-[16px]`,
    ...
    pcFilterIconContainer: {
        root: `absolute top-[50%] -mt-2 leading-none end-3 z-1`
    },
...
border: var(--border-1, 1px) solid var(--color-primary-Primary, #6C43C6);
background: var(--color-state-primary-Primary-Container-Pressed, rgba(208, 188, 255, 0.18)); */
  .label{
